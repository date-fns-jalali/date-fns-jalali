...
describe('addWeeks', function() {
  it('adds the given number of weeks', function() {
    ...
    assert.deepStrictEqual(result, /* 1393/7/7 */ new Date(2014, 8 /* Sep */, 29))
  })

  it('accepts a timestamp', function() {
    const result = addWeeks(/* 1393/6/10 */ new Date(2014, 8 /* Sep */, 1).getTime(), 1)
    assert.deepStrictEqual(result, /* 1393/6/17 */ new Date(2014, 8 /* Sep */, 8))
  })

  it('converts a fractional number to an integer', function() {
    const result = addWeeks(/* 1393/6/10 */ new Date(2014, 8 /* Sep */, 1), 4.95)
    assert.deepStrictEqual(result, /* 1393/7/7 */ new Date(2014, 8 /* Sep */, 29))
  })
  ...
  it('returns `Invalid Date` if the given amount is NaN', function() {
    const result = addWeeks(/* 1393/6/10 */ new Date(2014, 8 /* Sep */, 1), NaN)
    // @ts-expect-error
    assert(result instanceof Date && isNaN(result))
  })
