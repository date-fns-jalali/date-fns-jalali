...
export function isLeapGregorianYear(gy: number) {
...
*/
export function d2j(julianDay: number) {
  if (isNaN(julianDay)) {
 ...
 * @param month
 */
export function normalizeMonth(year: number, month: number): [number, number] {
  month = month - 1;
  if (month < 0) {
...
function pmod(a: number, b: number) {
