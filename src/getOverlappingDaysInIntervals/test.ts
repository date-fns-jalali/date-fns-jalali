...
describe("getOverlappingDaysInIntervals", () => {
  ...
  describe("when the time intervals overlap", () => {
    ...
    it("considers equal 0-lenght intervals not overlapping", () => {
      const date = /* 1395/8/25 */ new Date(2016, 10, 15);
      const numOverlappingDays = getOverlappingDaysInIntervals(
        { start: date, end: date },
        { start: date, end: date },
      );
    ...
    it("considers equal 1ms-length intervals overlapping", () => {
      const start = /* 1395/8/25 */ new Date(2016, 10, 15);
      const end = /* 1395/8/25 */ new Date(2016, 10, 15, 0, 0, 0, 1);
      const numOverlappingDays = getOverlappingDaysInIntervals(
        { start, end },
        { start, end },
      );
  ...
  it("properly sorts the dates", () => {
    const result = getOverlappingDaysInIntervals(
      {
        start: /* 1380/6/10 */ new Date(2001, 8 /* Sep */, 1, 16),
        end: /* 1402/9/29 */ new Date(2023, 11 /* Dec */, 20, 16),
      },
      {
        start: /* 1402/9/30 */ new Date(2023, 11 /* Dec */, 21, 16),
        end: /* 1380/6/18 */ new Date(2001, 8 /* Sep */, 9, 16),
      },
    );
